...
PHP_ARG_WITH(tipi_globals_demo, for tipi_globals_demo support,
dnl Make sure that the comment is aligned:
[  --with-tipi_globals_demo             Include tipi_globals_demo support])

dnl Otherwise use enable:

dnl PHP_ARG_ENABLE(tipi_globals_demo, whether to enable tipi_globals_demo support,
dnl Make sure that the comment is aligned:
dnl [  --enable-tipi_globals_demo           Enable tipi_globals_demo support])
...
if test "$PHP_TIPI_GLOBALS_DEMO" != "no"; then
  ...
  PHP_NEW_EXTENSION(tipi_globals_demo, tipi_globals_demo.c, $ext_shared)
fi
